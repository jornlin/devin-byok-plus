...
Devin Desktop BYOK 增强版 — 多模型路由、双槽位、思考强度控制，使用自己的 API Key 连接 Claude / GPT / Gemini 模型。
...
> **💡 自动保存提示**：配置字段（API Key、模型、端口等）会在输入后自动保存（650ms 防抖），无需手动点击"保存配置"按钮。显式保存仍会显示成功提示。
...
2. **通过命令行**：`code --install-extension devin-byok-plus-2.0.2.vsix`
...
详细结构说明请查看：
- [docs/STRUCTURE.md](./docs/STRUCTURE.md) - 整体项目结构
- [docs/PROXY_STRUCTURE.md](./docs/PROXY_STRUCTURE.md) - Proxy 目录结构与构建流程
...
1. 源代码位于 `src/` 目录
2. 构建后产物输出到 `dist/extension.js`（单文件打包）
3. VSIX 安装包生成到 `build/` 目录
