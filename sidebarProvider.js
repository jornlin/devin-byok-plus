...
const KEY_AUTO_START_PROXY = "windsurf-byok-bridge.autoStartProxy";
const KEY_PATCH_EXTENSION_PATH = "windsurf-byok-bridge.patchExtensionPath";
const DEFAULT_SYSTEM_PROMPT = ["You are `Cascade`, Windsurf's software engineering assistant.", "Help the user solve coding tasks through implementation, debugging, code review, and repository-aware reasoning.", "Prioritize correctness, low-risk changes, and forward progress."].join("\n");
const _0x5a96b6 = {
  id: "default",
  label: "默认工程助手",
...
class SidebarProvider {
  ...
  resolveWebviewView(_0x10e606) {
    ...
    try {
      ...
      vscode.window.showErrorMessage("Windsurf BYOK Bridge 控制面板加载失败：" + (_0x31934e instanceof Error ? _0x31934e.message : String(_0x31934e)));
    }
  ...
  getStoredPatchExtensionPath() {
    const _0x197f2a = this.context.globalState.get(KEY_PATCH_EXTENSION_PATH);
  ...
  async readWindsurfProcessCommandLines() {
    ...
    if (process.platform === "win32") {
      const _0x9e9f2 = "$self=$PID; Get-CimInstance Win32_Process | Where-Object { $_.ProcessId -ne $self -and $_.CommandLine -match '(?i)(windsurf|codeium|language_server)' } | ForEach-Object { $_.CommandLine }";
      try {
    ...
    return _0x2264d8.split(/\r?\n/).map(_0x438a34 => _0x438a34.trim()).filter(_0x50a4dc => /(windsurf|codeium|language_server|windsurf-server)/i.test(_0x50a4dc)).filter(_0x2fc16a => !/Get-CimInstance Win32_Process|wmic\.exe process get CommandLine/i.test(_0x2fc16a));
  }
  async checkWindsurfProcessRouting(_0x3c0348) {
    ...
    try {
      ...
      return this.envCheckItem("windsurf-process-routing", "进程路由参数", "warning", "无法读取 Windsurf/Codeium 进程命令行：" + _0x4a3ba6, false);
    }
    if (_0xb0cce.length === 0) {
      return this.envCheckItem("windsurf-process-routing", "进程路由参数", "ok", "未检测到运行中的 Windsurf/Codeium/language_server 进程；启动后可再次验证", false);
    }
    ...
    if (_0x3cd4f7 && _0x4d87f2) {
      return this.envCheckItem("windsurf-process-routing", "进程路由参数", "ok", "检测到 " + _0xb0cce.length + " 个相关进程，命令行包含 Hybrid " + _0x5df13c + " 与 Inference " + _0xc74133, false);
    }
    const _0x5f1972 = [_0x3cd4f7 ? "Hybrid " + _0x5df13c : "", _0x4d87f2 ? "Inference " + _0xc74133 : ""].filter(Boolean).join("、") || "未发现本地代理端口";
    const _0x1fd7b8 = _0x18c29b ? "补丁已就绪但当前进程命令行未完整体现本地端口，建议重载/重启 Windsurf 后复查" : "补丁未完全就绪，建议先安装补丁并重载 Windsurf";
    return this.envCheckItem("windsurf-process-routing", "进程路由参数", "warning", "检测到 " + _0xb0cce.length + " 个相关进程；" + _0x5f1972 + "；" + _0x1fd7b8, false);
  }
  ...
  async checkManagedEnvironment() {
    ...
    _0x249fb8.push(this.envCheckItem("windsurf-patch", "Windsurf 补丁", _0x34ba01.length === 0 ? "ok" : "warning", !_0x1e98a3.path ? "未找到 Windsurf extension.js" : _0x34ba01.length === 0 ? "已安装" : "未就绪 " + _0x34ba01.length + "/" + _0x1e98a3.patches.length, _0x1bf5e2));
    return {
  ...
  async createDiagnosticReport() {
    ...
    return ["# Windsurf BYOK Bridge 诊断报告", "", "生成时间：" + new Date().toLocaleString(), "", "## 版本与宿主", this.jsonBlock({
      extension: _0x39db9b,
      host: _0x1f9fe0,
      windsurfProduct: _0x2cb94a,
      os: {
      ...
      host: "",
      health: _0x72ffb1
    }), "", "## 环境检测", _0x1cfa6d || "无检测项", "", "## Windsurf / Codeium 进程", processError ? this.textBlock(processError) : this.textBlock(processLines.join("\n")), "## 最近 100 行日志", this.textBlock(_0x15bc4d), ""].join("\n");
  }
  ...
  async ensurePatchAppliedAfterProxyStart(_0x406fb2 = true) {
    ...
    const _0x250a94 = "检测到 Windsurf 补丁丢失，已自动恢复 " + _0x4157b5.applied + " 个，需重载窗口生效";
  ...
  async forceRestartLanguageServer() {
    ...
    const _0x3e0678 = await vscode.window.showWarningMessage("将强制结束 Windsurf/Codeium 的 language_server/Codeium 子进程，Windsurf 会自动拉起新的 LS。是否继续？", {
      modal: true
    }, "强制重启 LS");
    ...
    const _0x4c0489 = "已结束 " + _0x30e9d0.length + " 个 LS/Codeium 进程，Windsurf 会自动重启 LS；如未恢复请点“重载窗口”";
  ...
  async clearWindsurfCache() {
    const _0x1d6d10 = await vscode.window.showWarningMessage("将关闭 Windsurf/Codeium，只清理运行缓存目录；不会删除 Windsurf 历史记录、工作区数据、聊天记录或配置。是否继续？", {
      modal: true
    }, "安全清理缓存");
    if (_0x1d6d10 !== "安全清理缓存") {
      return;
    }
    const _0x1b5d1e = path.join(os.tmpdir(), "windsurf-byok-bridge-clear-cache-" + Date.now() + (process.platform === "win32" ? ".cmd" : ".sh"));
    if (process.platform === "win32") {
      fs.writeFileSync(_0x1b5d1e, ["@echo off", "timeout /t 1 /nobreak >nul 2>&1", "taskkill /f /im Windsurf.exe >nul 2>&1", "taskkill /f /im language_server.exe >nul 2>&1", "taskkill /f /im codeium.exe >nul 2>&1", "taskkill /f /im Codeium.exe >nul 2>&1", "timeout /t 2 /nobreak >nul 2>&1", "echo 正在安全清除运行缓存...", "rd /s /q \"%APPDATA%\\Windsurf\\Cache\" >nul 2>&1", "rd /s /q \"%APPDATA%\\Windsurf\\CachedData\" >nul 2>&1", "rd /s /q \"%APPDATA%\\Windsurf\\CachedExtensionVSIXs\" >nul 2>&1", "rd /s /q \"%APPDATA%\\Windsurf\\Code Cache\" >nul 2>&1", "rd /s /q \"%APPDATA%\\Windsurf\\DawnCache\" >nul 2>&1", "rd /s /q \"%APPDATA%\\Windsurf\\GPUCache\" >nul 2>&1", "rd /s /q \"%LOCALAPPDATA%\\Windsurf\\Cache\" >nul 2>&1", "rd /s /q \"%LOCALAPPDATA%\\Windsurf\\CachedData\" >nul 2>&1", "rd /s /q \"%LOCALAPPDATA%\\Windsurf\\Code Cache\" >nul 2>&1", "rd /s /q \"%LOCALAPPDATA%\\Windsurf\\DawnCache\" >nul 2>&1", "rd /s /q \"%LOCALAPPDATA%\\Windsurf\\GPUCache\" >nul 2>&1", "rd /s /q \"%TEMP%\\codeium\" >nul 2>&1", "echo 运行缓存已清除完毕，Windsurf 历史记录已保留，请重新打开 Windsurf", "pause", ""].join("\r\n"), "utf-8");
    } else {
      fs.writeFileSync(_0x1b5d1e, ["#!/bin/sh", "sleep 1", "pkill -f \"Windsurf\" >/dev/null 2>&1 || true", "pkill -f \"Codeium\" >/dev/null 2>&1 || true", "pkill -f \"codeium\" >/dev/null 2>&1 || true", "sleep 1", "echo 正在安全清除运行缓存...", "rm -rf \"$HOME/Library/Caches/Windsurf\"", "rm -rf \"$HOME/Library/Caches/Codeium\"", "rm -rf \"$HOME/.cache/Windsurf\"", "rm -rf \"$HOME/.cache/Codeium\"", "rm -rf \"${TMPDIR:-/tmp}/codeium\"", "rm -rf \"/tmp/codeium\"", "echo 运行缓存已清除完毕，Windsurf 历史记录已保留，请重新打开 Windsurf", "printf \"Press Enter to close...\"", "read _", ""].join("\n"), "utf-8");
      fs.chmodSync(_0x1b5d1e, 493);
    }
    this.runDetachedCacheCleaner(_0x1b5d1e);
    this.postActionState("config", "success", "安全清理缓存脚本已启动，历史记录会保留，Windsurf 即将关闭");
  }
  ...
  async openPromptTemplatePicker() {
    ...
    const _0x528d3f = {
      label: "关闭提示词覆盖",
      description: "恢复使用 Windsurf 原始系统提示词",
      action: "disable"
    };
      ...
      case "saveConfig":
        {
          ...
          if (_0x1b9939.running) {
            ...
            if (_0x22bb31) {
              ...
              this.postActionState("patch", "success", "端口已变更；如需让 Windsurf 使用新端口，请手动点击“安装补丁”并重载窗口");
            } else {
              const _0x1a3d59 = {
                hybridPort: _0x4a048f,
                inferencePort: _0x16f1f4
      ...
      case "locateExtJs":
        {
          const _0x4b52f0 = await vscode.window.showOpenDialog({
            ...
            title: "选择 Windsurf extension.js"
          });
