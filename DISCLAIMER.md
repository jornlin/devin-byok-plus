...
本仓库部分代码经**故意混淆**处理，以限制非本地场景的随意复用与二次分发。这**不代表**隐瞒恶意行为；本项目按 MIT 许可发布，但：

- 混淆代码的**可读性、可审计性与可维护性**由使用者自行评估；
- 维护者**不提供**对混淆后内部实现逐行解释的支持义务；
- Fork 或二次打包时，你仍须遵守本免责声明及 MIT License 中的责任限制。
